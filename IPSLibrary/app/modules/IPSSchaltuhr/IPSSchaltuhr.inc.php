...
//			if ($Profile[$Value]['Color'] == -1 or $Profile[$Value]['Color'] == 0xFF0000) {
	...
	function get_CirclyNameByID($CircleId) {
	   $CircleItem       = IPS_GetName($CircleId);
		...
		return $ZSUConfig[$CircleItem][c_Property_IPSSchaltuhrName];
	}
	...
	function set_Overview($CircleId){
		...
		$html.="<table class='zeitplan'>\n\n";
		//  Startsensoren
		if (count($property[c_Property_StartSensoren])>0) {
			$html.="	<tr>";
			...
			foreach ($property[c_Property_StartSensoren] as $ID=>$Data) {
					...
					$html.="$farbe_tag ".$Data[c_Property_IPSSchaltuhrName]."<br>".(GetValue($Data[c_Property_SensorID]) ? GetValue($Data[c_Property_SensorID]) : "0")."$suffix  ".$Data[c_Property_Condition]."  ".$Data[c_Property_Value]." $suffix</td>\n";
			}
			$html.="	</tr>";
		}
		...
		//  Runsensoren
		if (count($property[c_Property_RunSensoren])>0) {
			$html.="	<tr>";
		...
		//  Stopsensoren
		if (count($property[c_Property_StopSensoren])>0) {
			$html.="	<tr>";
		...
		if (get_ControlValue(c_Control_SollAusgang, $CircleId))  	$farbe_tag = "		<td style='width: 160px; background: #008000;' colspan='1' align='center'>";
		else $farbe_tag = "		<td style='width: 160px; background: #800000;' colspan='1' align='center'>";
				$html.="$farbe_tag ".c_WFC_SollZustand."</td>\n";

		if (get_ControlValue(c_Control_IstAusgang, $CircleId))  	$farbe_tag = "		<td style='width: 160px; background: #008000;' colspan='1' align='center'>";
		else $farbe_tag = "		<td style='width: 160px; background: #800000;' colspan='1' align='center'>";
